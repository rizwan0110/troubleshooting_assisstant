...
class Reranker:
    ...
    def rerank(self, query: str, chunks: List[ScoredChunk], top_k: int = 5) -> List[RerankedChunk]:
        if not chunks:
            return []
        ...
        for i, r in enumerate(reranked[:top_k]):
            logger.info(f"  #{i+1} | rerank: {r.rerank_score:.4f} | dense: {r.dense_score:.4f} | sparse: {r.sparse_score:.4f}")
